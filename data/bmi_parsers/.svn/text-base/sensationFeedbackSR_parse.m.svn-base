% SENSATIONFEEDBACKSR_PARSE 
%
% $Id: sensationFeedbackSR_parse.m 966 2010-10-29 07:20:17Z joey $
% 
% this function reads in a sensationFeedback behavior file
% processes the data and saves a _parsed file to speed up
% subsequent analysis

function sensationFeedbackSR_parse(f)
...
try
  [trial.use_icms_catch_trials] = dealify([trial.outcome_t],use_icms_catch_trials,'previous');
catch
  [trial.use_icms_catch_trials] = deal(0);
end
...
try
  [trial.use_noise] = dealify([trial.outcome_t],use_noise,'previous');
catch
  [trial.use_noise] = deal(0);
end

try
  [trial.use_ubiquitous_noise] = dealify([trial.outcome_t],use_ubiquitous_noise,'previous');
catch
  [trial.use_ubiquitous_noise] = deal(0);
end

try
  [trial.icms_current] = dealify([trial.outcome_t],icms_current,'previous');
catch
  [trial.icms_current] = deal(0);
end

try
  [trial.noise_current] = dealify([trial.outcome_t],noise_current,'previous');
catch
  [trial.noise_current] = deal(0);
end

try
  [trial.noise_cv] = dealify([trial.outcome_t],noise_coeff_variation,'previous');
catch
  [trial.noise_cv] = deal(0);
end


%[trial.use_vib] = dealify([trial.outcome_t],use_vibrator,'previous');
%trial.use_vis] = dealify([trial.outcome_t],use_visual_feedback,'previous');
try
...
% try
%   [trial.icms_chan_3_current] = dealify([trial.outcome_t],icms_chan_3_current,'previous');
% catch
%   [trial.icms_chan_3_current] = deal(-1);
% end

try
  [trial.use_icms_for_wrong_target] = dealify([trial.outcome_t],use_icms_for_wrong_target,'previous');
catch
  [trial.use_icms_for_wrong_target] = deal(0);
end

try
  [trial.use_decay_icms_current] = dealify([trial.outcome_t],use_decay_icms_current,'previous');
catch
  [trial.use_decay_icms_current] = deal(0);
end

try
  [trial.icms_duration] = dealify([trial.outcome_t],stim_duration,'previous');
catch
  [trial.icms_duration] = deal(-1);
end
...
%  
%   was_in_cloud = zeros(num_targets,1);
%   
%   time_in_cloud = zeros(num_targets,1);
%   time_in_target = zeros(num_targets,1);
...
%         if ~isempty(in_state_seq)
%           if in_state_seq(end) ~= trial(i).target_ids(k)
%             in_state_seq   = [in_state_seq ; trial(i).target_ids(k)];
%             in_state_seq_t = [in_state_seq_t ; trial(i).cursor(1,j)];
%           end
%         else
%           in_state_seq   = [in_state_seq ; trial(i).target_ids(k)];
%           in_state_seq_t = [in_state_seq_t ; trial(i).cursor(1,j)];
%         end
%         time_in_target(k) = time_in_target(k) + dt;
%       end
%       if cursor_in_target(tps{k},cp,clr,cur)
%         if ~isempty(cloud_state_seq)
%           if cloud_state_seq(end) ~= trial(i).target_ids(k)
%             cloud_state_seq   = [cloud_state_seq ; trial(i).target_ids(k)];
%             cloud_state_seq_t = [cloud_state_seq_t ; trial(i).cursor(1,j)];
%           end
%         else
%           cloud_state_seq   = [cloud_state_seq ; trial(i).target_ids(k)];
%           cloud_state_seq_t = [cloud_state_seq_t ; trial(i).cursor(1,j)];
...
%       else
%         if was_in_cloud(k)
%           if ~isempty(cloud_state_seq_out)
%             if cloud_state_seq_out(end) ~= trial(i).target_ids(k)
%               cloud_state_seq_out = ...
% 	        [cloud_state_seq_out ; trial(i).target_ids(k)];
%               cloud_state_seq_out_t = ...
% 	        [cloud_state_seq_out_t ; trial(i).cursor(1,j)];
%             end
%           else
%             cloud_state_seq_out = ...
%               [cloud_state_seq_out ; trial(i).target_ids(k)];
...
%   trial(i).time_in_target = time_in_target;
% end
% 
